...
class Router:
    ...
    def print_table(self):
        ...
        os.system('clear')
        print()
        print(f"| {'-'*5} | {'-'*20} | {'-'*20} | {'-'*10} | {'-'*25} |")
        print(f"| {'Id':<5} | {'Destination':<20} | {'Output Interface':<20} | {'Metric':<10} | {'Time Stamp':<25} |")
        print(f"| {'-'*5} | {'-'*20} | {'-'*20} | {'-'*10} | {'-'*25} |")
        for destination, values in self.routing_table.items():
            ...
            print(f"| {id:<5} | {destination:<20} | {output_interface:<20} | {metric:<10} | {time_stamp:<30} |")
        print(f"| {'-'*5} | {'-'*20} | {'-'*20} | {'-'*10} | {'-'*25} |")

    def update_routing_table(self, destination, output_interface, metric, connection, id, time_stamp):
        # update the routing table with new information about a destination
        if destination not in self.routing_table:
            print(f"Connected to Mock Router #{id}")
            self.routing_table[destination] = {'output_interface': output_interface, 'metric': metric, 'connection': connection, 'id': id, 'time_stamp': time_stamp}
    ...
    def handle_routing_packet(self, data, connection):
        ...
        destination, output_interface, metric, id, time_stamp = data.split(',')
        self.update_routing_table(destination, output_interface, int(metric), connection, id, time_stamp)
